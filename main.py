...
def read_html_tags():
    all_tags = []
    for row in sheet_read2.iter_rows(min_row=5, max_row=5, min_col=2, max_col=4):
        for cell in row:
...
def calkueted():
    ...
    tag2 = 1
    tag3 = 2
    for row in sheet_read.iter_rows(min_row=5, max_row=5, min_col=2, max_col=15):
        for cell in row:
            read_cell = cell.value
            if read_cell == 'x':
                new_sheet.cell(row=next_row, column=next_column).value = 'x' # винести за if
            else:
                url = read_cell
                print(url)
                try:
